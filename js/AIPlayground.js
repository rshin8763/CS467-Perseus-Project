...
Perseus.game = new Phaser.Game(1000, 550, Phaser.AUTO, '', {preload:preload, update:update, create:create});
...
//{
    ...
    function create()
    {
        ...
        pause_button.events.onInputUp.add(Perseus.pause);
 ...
 ** UPDATES AMOUNT ON SCREEN
 *****/
Perseus.updateStone = function(x)
{
    stone = stone + x;
 ...
 *****/
Perseus.updateThisHealth = function(x)
{
 ...
 ** UPDATES AMOUNT ON SCREEN. IF HEALTH IS <= 0, GAME ENDS
 *****/
Perseus.updateEnemyHealth = function(x)
{
 ...
 ** RETURNS NOTHING
 *****/
Perseus.newGame = function()
{
    Perseus.unpause();
}

Perseus.muteMusic = function()
{
    if(music.mute == true)
    {
        music.mute = false;
